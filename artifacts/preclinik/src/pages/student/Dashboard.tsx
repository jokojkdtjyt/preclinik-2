...
import { useListModules, useGetProgress, useListPurchased } from '@workspace/api-client-react';
...
export default function Dashboard() {
  ...
  const { data: purchased } = useListPurchased();

  const ownedCount = purchased?.length ?? 0;
        ...
        <div className="bg-white rounded-[22px] p-8 md:p-12 border border-border shadow-sm flex items-center justify-between overflow-hidden relative" style={{ boxShadow: 'var(--shadow)' }}>
          ...
          <div className="relative z-10 max-w-2xl space-y-4">
            ...
            <p className="text-lg text-muted-foreground">
              {ownedCount > 0
                ? `You own ${ownedCount} module${ownedCount !== 1 ? 's' : ''} and have completed ${completedCount} lesson${completedCount !== 1 ? 's' : ''} so far.`
                : 'Start your preclinical journey — browse the catalog and enrol in your first module.'}
            </p>
            <div className="pt-4">
              {ownedCount > 0 ? (
                ...
                <Link href="/catalog" className="inline-flex items-center gap-2 bg-primary hover:bg-primary/90 text-white font-bold px-6 py-3 rounded-xl transition-all shadow-lg shadow-primary/20">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-[22px] border border-border flex items-start gap-4 hover:-translate-y-1 transition-transform" style={{ boxShadow: 'var(--shadow-sm)' }}>
          ...
          <div>
            ...
            <div className="text-3xl font-serif font-bold">{ownedCount}</div>
          </div>
        </div>
        <div className="bg-white p-6 rounded-[22px] border border-border flex items-start gap-4 hover:-translate-y-1 transition-transform" style={{ boxShadow: 'var(--shadow-sm)' }}>
          ...
          <div>
            ...
            <div className="text-3xl font-serif font-bold">{completedCount}</div>
          </div>
        </div>
      </section>
