  ...
  const handleCartToggle = (e: React.MouseEvent) => {
    ...
    if (inCart) {
      removeFromCart.mutate(
        ...
        {
          onSuccess: (data) => {
            // Update cache immediately so badge + button flip without waiting for refetch.
  ...
  return (
    ...
    >
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        {/* Progress row (owned modules) or meta row (unowned) */}
        {isPurchased ? (
          <div className="mb-4">
        ...
        {/* Footer: left = price/owned badge, right = action buttons */}
        <div className="flex items-center justify-between mt-auto pt-4 border-t border-border">
          {/* Left slot */}
          {isLibrary ? (
            // Library mode: no price, no badge — the context makes ownership obvious
            <span />
          ) : isPurchased ? (
          ...
          <div className="flex items-center gap-2">
            {/* Cart toggle — store mode, not yet owned */}
            {!isLibrary && !isPurchased && (
              <button
